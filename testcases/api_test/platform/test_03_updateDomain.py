...
from operation.platform import updateDomain
...
class TestUpdateDomain():
    @allure.story("接口03--编辑逻辑域")
    @allure.description("该用例是编辑逻辑域数据")
    ...
    @pytest.mark.parametrize(
        "id, domainName, domainCnName, description, platformPid, except_code, except_result ,"
        "except_msg,except_testcase_description",
        api_data["test_platform_updateDomain"])
    def test_platform_updateDomain(self, id, domainName, domainCnName, description, platformPid, except_code,
                                   except_result, except_msg
                                   , except_testcase_description):
        logger.info("*************** 开始执行用例 ***************")
        # 调用operation中的query_by_page方法
        result = updateDomain(id, domainName, domainCnName, description, platformPid)
        excel_oper.write_data(result, "SOA服务管理-平台", "编辑逻辑域", except_code, result.success == except_result,
                              except_result)
...
if __name__ == '__main__':
    # # 执行用例并生成allure测试报告
    pytest.main(['-s', './test_03_updateDomain.py', '--alluredir', './result/'])

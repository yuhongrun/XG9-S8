...
from operation.platform import batchUpdateField
...
class TestBatchUpdateField():
    @allure.story("接口08--删除子系统")
    @allure.description("该用例是删除子系统")
    ...
    @pytest.mark.parametrize(
        "ids,isDelete,except_code,except_result,except_msg, except_testcase_description",
        api_data["test_platform_batchUpdateField"])
    def test_platform_batchUpdateField(self, ids, isDelete, except_code,
                                       except_result, except_msg, except_testcase_description):
        logger.info("*************** 开始执行用例 ***************")
        # 调用operation中的query_by_page方法
        result = batchUpdateField(ids, isDelete)
        excel_oper.write_data(result, "SOA服务管理-平台", "删除子系统", except_code, result.success == except_result,
                              except_result)
...
if __name__ == '__main__':
    # # 执行用例并生成allure测试报告
    pytest.main(['-s', './test_08_batchUpdateField.py', '--alluredir', './result/'])

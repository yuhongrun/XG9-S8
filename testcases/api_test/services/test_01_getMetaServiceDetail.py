...
from operation.services import getMetaServiceDetail
...
class TestGetPlatformList():
    @allure.story("接口01--服务详情")
    @allure.description("该用例是根据服务主键ID查询服务详情")
    @allure.title("测试数据：【{except_testcase_description}】")
    @pytest.mark.parametrize(
        "id,except_code,except_result,except_testcase_description",
        api_data["test_services_getMetaServiceDetail"])
    def test_services_getMetaServiceDetail(self, id, except_code,
                                           except_result, except_testcase_description):
        logger.info("*************** 开始执行用例 ***************")
        # 调用operation中的query_by_page方法
        result = getMetaServiceDetail(id)
        excel_oper.write_data(result, "SOA服务管理-服务", "服务详情", except_code, result.success == except_result,
                              except_result)
        # 断言返回的结果是否等于yml,定义的值
...
if __name__ == '__main__':
    # # 执行用例并生成allure测试报告
    pytest.main(['-s', './test_01_getMetaServiceDetail.py'])

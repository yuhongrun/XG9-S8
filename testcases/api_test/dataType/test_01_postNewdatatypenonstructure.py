...
from operation.data_type import postNewdatatypenonstruche
from testcases.conftest import api_data


@allure.epic("XG9-S8系统接口自动化测试")
@allure.feature("管理后台--数据类型")
class TestPostNewdatatypenonstructure():
    @allure.story("接口01--新增数据类型非结构体")
    @allure.description("该用例是新增数据类型非结构体")
    ...
    @pytest.mark.parametrize(
        ...
        api_data["test_dataType_postNewdatatypestructure"])
    def test_post_New_data_type_no_nstruche(self, name, description, category, baseDatatype, namePrefix,discretes, resolution, except_code,
                                            except_result, except_testcase_description):
        logger.info("*************** 开始执行用例 ***************")
        # 调用operation中的query_by_page方法
        result = postNewdatatypenonstruche(name, description, category, baseDatatype, namePrefix,discretes,resolution)
        excel_oper.write_data(result, "管理后台--数据类型", "新增数据类型非结构体", except_code,
...
if __name__ == '__main__':
    # # 执行用例并生成allure测试报告
    pytest.main(['-s', './test_01_postNewdatatypenonstructure.py', '--alluredir', './result/'])

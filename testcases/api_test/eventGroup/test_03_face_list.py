...
from operation.eventGroup import face_list
...
class TestFaceList():
    ...
    @pytest.mark.parametrize("pageIndex, pageSize , eventGroupPid, except_result,except_code, except_testcase_description",
                             api_data["test_eventGroup_face_list"])
    def test_face_list(self, pageIndex, pageSize, eventGroupPid, except_result, except_code, except_testcase_description):
        logger.info("*************** 开始执行用例 ***************")
        result = face_list(pageIndex, pageSize, eventGroupPid)
...
if __name__ == '__main__':
    ...
    # # 执行用例并生成allure测试报告
    pytest.main(['-s', './test_03_face_list.py', '--alluredir', './result/'])

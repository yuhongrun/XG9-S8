...
from operation.element_model import add_OrUpdate_Linked_Element
from testcases.conftest import api_data


@allure.epic("XG9-S8系统接口自动化测试")
@allure.feature("车型元模型管理")
class Test_add_OrUpdateLinkedElement():
    @allure.story("接口05-车元模型—关联管理—保存关联元素")
    ...
    @pytest.mark.parametrize("series_code, model_code, platform_id, linkId, linkType, metaTyp, preId, except_result, "
                             "except_code, except_testcase_description",
                             api_data["test_05_add_OrUpdate_Linked_Element"])
    # series_code, model_code, platform_id
    def test_add_OrUpdateLinkedElement(self, series_code, model_code, platform_id, linkId, linkType, metaTyp, preId, except_result, except_code, except_testcase_description):
        logger.info("*************** 开始执行用例 ***************")
        # 调用operation中的query_by_page方法
        result = add_OrUpdate_Linked_Element(series_code, model_code, platform_id, linkId, linkType, metaTyp, preId)
...
if __name__ == '__main__':
    # # 执行用例并生成allure测试报告
    pytest.main(['-s', './test_05_addOr_Update_Linked_Element.py', '--alluredir', './result/'])

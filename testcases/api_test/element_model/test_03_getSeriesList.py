...
from operation.element_model import PostSeriesList
...
class TestPostSeriesList():
    @allure.story("接口03-车元模型—车系列表")
    @allure.description("该用例是查询车系数据")
    ...
    @pytest.mark.parametrize("page_index,page_size,series_code,series_name,except_result,except_code, except_testcase_description",
                             api_data["test_03_element_model_getSeriesList"])
    def test_element_model_PostSeriesList(self, page_index, page_size, series_code, series_name,except_result, except_code, except_testcase_description):
        logger.info("*************** 开始执行用例 ***************")
        # 调用operation中的query_by_page方法
        result = PostSeriesList(page_index, page_size, series_code, series_name)
        excel_oper.write_data(result, "车型元模型管理", "车系列表", except_code,
...
if __name__ == '__main__':
    # # 执行用例并生成allure测试报告
    pytest.main(['-s', './test_03_getSeriesList.py', '--alluredir', './result/'])
